...
function posts(){
 let p=null;
 try{p=JSON.parse(localStorage.getItem(postsKey)||"null")}catch(e){p=null}
 if(!Array.isArray(p)||!p.length){
   p=Array.isArray(SEED_POSTS)?SEED_POSTS.slice():[];
   try{localStorage.setItem(postsKey,JSON.stringify(p))}catch(e){}
 }
 return p;
}
